...
struct PentominoOutline : Codable, Identifiable{
    ...
    static let pentominoes : [PentominoOutline]? = {
        return pentominoData()
    }()
...
struct PuzzleOutline : Codable, Identifiable{
    ...
    static let puzzles : [PuzzleOutline]? = {
        return puzzleData()
    }()
}

extension PuzzleOutline {
    static func puzzleData() -> [PuzzleOutline]? {
        guard let url = Bundle.main.url(forResource: "PuzzleOutlines", withExtension: "json") else { return nil }
        let data : [PuzzleOutline]?
        do {
            let contents = try Data(contentsOf: url)
            let decoder = JSONDecoder()
            data = try decoder.decode([PuzzleOutline].self, from: contents)
        } catch {
            print(error)
            data = nil
        }
...
extension PentominoOutline {
    static func pentominoData() -> [PentominoOutline]? {
        guard let url = Bundle.main.url(forResource: "PentominoOutlines", withExtension: "json") else { return nil }
        let data : [PentominoOutline]?
        do {
            let contents = try Data(contentsOf: url)
            let decoder = JSONDecoder()
            data = try decoder.decode([PentominoOutline].self, from: contents)
        } catch {
            print(error)
            data = nil
        }
